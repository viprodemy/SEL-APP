...
export default function EmotionCheckInFlow() {
  ...
  const handleFinish = async () => {
    ...
    setStep(8); // Move to Power Card early so student isn't stuck on Post-Emotion page
    ...
    // Rest of handling will happen in useEffect triggered by step 8
  }

  const performFinalProcessing = async () => {
    ...
    try {
        ...
        const aiReportText = result.report;
        ...
        toast({
            variant: "destructive",
            title: "Error / 错误",
            description: "Something went wrong saving your insights. / 记录同步时发生错误。",
        });
  ...
  useEffect(() => {
    if (step === 8 && isFinalizing && queue.isProcessing && !processingRef.current) {
